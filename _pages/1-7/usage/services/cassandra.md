...
~~~ $ dcos cassandra connection { "address": [ "10.0.2.136:9042", "10.0.2.138:9042", "10.0.2.137:9042" ], "dns": [ "node-0.cassandra.mesos:9042", "node-1.cassandra.mesos:9042", "node-2.cassandra.mesos:9042" ]

} ~~~

*   Step 3. [SSH into a DC/OS node][2]:
...
~~~ $ cat cassandra1.json { "service": { "name": "cassandra1" } }
...
Then, use the [framework cleaner script][8] to remove your Cassandra instance from Zookeeper and destroy all data associated with it. The arguments the script requires are derived from your service name:
...
~~~ { "address": [ "10.0.0.47:9042", "10.0.0.50:9042", "10.0.0.49:9042" ], "dns": [ "node-0.cassandra.mesos:9042", "node-1.cassandra.mesos:9042", "node-2.cassandra.mesos:9042" ]
...
List addresses = Arrays.asList( new InetSocketAddress("10.0.0.47", 9042), new InetSocketAddress("10.0.0.48", 9042), new InetSocketAddress("10.0.0.45", 9042));
...
To view general information about a node, the following command my be run from the CLI. `$ dcos cassandra --name=<service-name> node describe <nodeid>` In contrast to the status command, this command requests information from the DC/OS Cassandra Service and not the Cassandra node.
